...
if(isset($_POST['submit'])) {
    ...
    $query = "UPDATE tblservices SET ServiceName='$sername', Description='$des', Cost='$cost'";
    
    if (!empty($_FILES['serviceImage']['name'])) {
        $serviceImage = $_FILES["serviceImage"]["name"];
        $extension = substr($serviceImage, strlen($serviceImage) - 4, strlen($serviceImage));
        $allowed_extensions = array(".jpg",".jpeg",".png",".gif");
        
        if (!in_array($extension, $allowed_extensions)) {
            echo "<script>alert('Invalid format. Only jpg / jpeg/ png /gif format allowed');</script>";
        } else {
            $serviceImage = md5($serviceImage) . time() . $extension;
...
<html>
<head>
<title>Streamlined Salon Management System | Update Services</title>
...
<body class="cbp-spmenu-push">
    ...
    <div class="form-group"> 
        ...
        <input class="form-control" name="des" id="des" rows="5" required="true" value="<?php echo $row['Description'];?>"> 
    </div>
    <div class="form-group"> 
    ...
    <div class="form-group">
        <img src="images/<?php echo $row['Image']; ?>" width="100" height="100" alt="Service Image">
    </div>
    <?php } ?>
    <?php } ?>
